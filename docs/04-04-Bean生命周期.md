...
上一章，我们已经了解了容器是如何将 XML 配置文件中的 `<bean>` 标签解析并注册到 `DefaultListableBeanFactory` 的 `beanDefinitionMap` 中，最终形成 `**beanName -> BeanDefinition**` 这样的映射关系。
...
第二行代码是我们开发者手动调用的 `getBean()` 方法，但是在**第一行代码的后半部分**，容器**可能就自己调用了许多次 **`**getBean()**`** 方法**。
...
但是不管单例还是原型 Bean，最终都会进入同一个入口方法：`**createBean()**`，由它负责后续的实例化和初始化过程。
...
在 Bean 实例初始化结尾，如果该 Bean 实现了某些 `**Aware**`** 接口**（例如 `BeanNameAware`、`BeanFactoryAware` 等），`initializeBean()` 方法内会给它注入一些额外的信息，例如：
