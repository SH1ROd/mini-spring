...
<img src="https://cdn.nlark.com/yuque/0/2025/png/12469155/1754852474148-80269523-fb54-40a2-bf5c-2cd3b6d8b90d.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_134%2Ctext_5Luj56CB6ZqP5oOz5b2V55-l6K-G5pif55CD%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10" width="2352" title="" crop="0,0,1,1" id="uc85f711d" class="ne-image">
...
代码运行到这里，逻辑已经很清晰了：`resourceLoader` 会根据路径的前缀来决定采用哪种方式处理
...
具体过程是：先把 `Resource` 解析成一个 DOM 文档对象，再遍历这个 DOM，把 XML 中 `<bean>` 标签的信息转化为 `**beanName -> BeanDefinition**` 的映射关系，存入容器内部的 `map` 结构。

这个步骤的实现位于 `**XmlBeanDefinitionReader**` 中，我们来看核心代码：
...
`BeanDefinitionDocumentReader` 的默认实现类是 `**DefaultBeanDefinitionDocumentReader**`。
...
这一步标志着：**XML 配置文件中的 **`**<bean>**`** 标签，已经被正式解析并注册到 **`**beanFactory**`** 的 **`**beanDefinitionMap**`** 中，形成 **`**beanName -> BeanDefinition**`** 的映射关系。**
